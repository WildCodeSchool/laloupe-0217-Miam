angular.module('app')
  .controller('MangerController', function($scope, $state, FoodFactory, LocalService, Auth, MangerService, GouterService, ProfilService, CurrentUser) {
    ...
    ProfilService.getAll().then(function(res) {
       ...
       function notEating(food) {
         return MangerService.findByProfile($scope.profile).then(function(res) {
          ...
          //  console.log("$scope.foodNotEaten", $scope.foodNotEaten);
       ...
       $scope.ok = function() {
         ...
         if ($scope.user.email !== undefined) {
           ...
           for (var j = 0; j < $scope.alimentsForDatabase.length; j++) {
             ...
             MangerService.likeAll($scope.alimentsForDatabase[j], $scope.profile).then(function(res) {
